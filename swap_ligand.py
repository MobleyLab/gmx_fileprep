...
def main():
    ...
    print("Step  3: Parameterizing Ligand with OpenFF")
    openff_ff = offw.setup_ff(ligand_ff)
    lig_mol = offw.create_molecule(ligand_mol2_solvent, solvent_ligcode)
    lig_positions = lig_mol.conformers[0]
    lig_topology = lig_mol.to_topology()
    lig_system = openff_ff.create_openmm_system(lig_topology)
    ...
    print("Step  5: Creating Parmed Structure for Solvent + Protein: Ligand + Solvent + Ions")
    mask = pmdw.create_mask_from_exclusion(pmd_solvent_struct, exclude_resids)
    pmd_solvent_struct = pmd_solvent_struct[':Na+:Cl-'] \
                            + pmd_lig_struct \
...
if __name__ == "__main__":
    ...
    output_path = desktop_path / "bace_prep"

    ligand_ff = output_path / "sage-2.1.0rc.offxml"

    complex_ligcode = "LIG"
    solvent_ligcode = "UNL"

    for ligfam in ["spirocycles", "amide_series", "biphenyl"]:
